...
32. 敲入 URL 到浏览器渲染完成的过程到底经历了什么
    - 输入地址
        - 在输入的时候，浏览器在智能的匹配url
        - 它会从历史记录，书签等处匹配已经输入的字符串，然后给出补全提示
        - 也就是说还没敲回车前，浏览器已经使用dns预取技术，解析域名了
        - 如果该域名有相关缓存，则直接显示页面

    - 查询dns
        - 浏览器会搜索自己的dns缓存
        - 没找到就到系统的缓存中查找，这一步也会查找本机的hosts文件，看看有没有对应的域名映射
        - 系统中也没有的话会到你的路由器中查找
        - 路由器中也没找到，会到服务器中递归查找
        - 域名服务器将查找出的ip地址返回给系统，并缓存起来
        - 然后系统再将ip地址给到浏览器，浏览器也会缓存
        - 至此，浏览器已经得到了域名对应的ip地址

    - 建立tcp连接
        - 经历tcp的三次握手，彼此确保其发送，和接收请求的能力
        - 互相确认成功后就可以发送请求了

    - 响应请求
        - 返回响应状态码，响应报文
        - 发送结束后，断开连接，经历tcp4次挥手s

    - 浏览器处理返回
        - 解析html，构建dom树
        - 解析css，构建css树
        - 遇到js文件时，html解析会被挂起，js引擎会处理js，涉及到js的事件循环机制
        - 合并dom树和css树，生成render树
        - 布局render树，涉及到回流，重绘，计算元素尺寸，位置
        - 绘制render树，绘制页面像素信息
        - 最终依靠gpu，展示页面

33. [前端经典面试题(60道前端面试题包含JS、CSS、React、网络、浏览器、程序题等)](https://segmentfault.com/a/1190000020391424)

34. [2021中高级前端面试题合集](https://www.jianshu.com/p/055e08887530)